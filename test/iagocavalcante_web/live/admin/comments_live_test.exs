defmodule IagocavalcanteWeb.Admin.CommentsLiveTest do
  ...
  describe "approve_comment event" do
    ...
    test "updates pending comments count after approval", %{conn: conn, comment: comment} do
      ...
      initial_count =
        html
        |> Floki.find("text()")
        |> Enum.filter(&String.contains?(&1, "pending comments"))
        |> length()
  ...
  describe "spam score display" do
    test "shows correct color coding for spam scores", %{conn: conn} do
      ...
      high_spam =
        comment_fixture(%{
          content:
            "Buy viagra cheap! http://spam1.com http://spam2.com http://spam3.com http://spam4.com http://spam5.com"
        })
  ...
  describe "auto-refresh functionality" do
    test "refreshes comments periodically" do
      ...
      {:noreply, updated_socket} =
        IagocavalcanteWeb.Admin.CommentsLive.handle_info(:refresh_comments, socket)
  ...
  describe "helper functions" do
    ...
    test "spam_score_color/1 returns correct CSS classes" do
      # High spam
      assert CommentsLive.spam_score_color(0.8) == "text-red-600"
      # Medium spam  
      assert CommentsLive.spam_score_color(0.5) == "text-orange-600"
      # Low spam
      assert CommentsLive.spam_score_color(0.2) == "text-green-600"
      # Boundary case
      assert CommentsLive.spam_score_color(0.7) == "text-red-600"
      # Boundary case
      assert CommentsLive.spam_score_color(0.4) == "text-orange-600"
    end
  end
